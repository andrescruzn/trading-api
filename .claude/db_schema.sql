...
CREATE TABLE `bots` (
  ...
  `feature_set_id` bigint DEFAULT NULL COMMENT 'Feature set de indicadores técnicos que usa este bot',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_bots_feature_set` (`feature_set_id`),
  CONSTRAINT `fk_bots_account` FOREIGN KEY (`account_id`) REFERENCES `accounts` (`id`),
  CONSTRAINT `fk_bots_feature_set` FOREIGN KEY (`feature_set_id`) REFERENCES `feature_sets` (`id`),
...
CREATE TABLE `signals` (
  ...
  `entry_price` decimal(30,12) DEFAULT NULL COMMENT 'Precio de entrada sugerido por el agente',
  `stop_loss` decimal(30,12) DEFAULT NULL COMMENT 'Nivel de stop loss calculado',
  `take_profit` decimal(30,12) DEFAULT NULL COMMENT 'Nivel de take profit calculado',
  `position_size` decimal(30,12) DEFAULT NULL COMMENT 'Tamaño de posición: capital × risk_pct / |entry-SL|',
  `rr_ratio` decimal(10,4) DEFAULT NULL COMMENT 'Ratio Riesgo/Recompensa (TP-entry)/(entry-SL)',
  `approved` tinyint(1) NOT NULL DEFAULT '1' COMMENT '1=aprobada por todos los filtros, 0=rechazada',
